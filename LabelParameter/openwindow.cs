...
namespace NewJeans
{
    ...
    public class LabelParameter : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            var handler = new FamilyParameterHandler();
            var externalEvent = ExternalEvent.Create(handler);
            var control = new LabelParameterResult(externalEvent, handler);

            _existingWindow = new Window
            {
                Title = "Lable Parameter 추출",
                Content = control,
                Width = 450,
                Height = 500,
